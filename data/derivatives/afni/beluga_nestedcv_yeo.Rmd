...
In this model, the probability of retrieving a memory m at time t is proportional to its activation A(m, t), a scalar quantity that reflects a memory’s absolute availability. As in the multiple trace theory (Moscovitch et al., 2005; Nadel et al., 2000), each memory m is associated with multiple episodic traces, each of which corresponds to a specific event in which m was retrieved or (re)encoded, and each of which decays over time.


## Example

This figure below shows the relationship between activation of a memory and the odds of its component traces. It tracks the activation over time of a memory that has been encoded in three different episodes, at times t = 1, 18, and 75 seconds. Each episode leaves an independent fading trace with a characteristic decay rate (colored lines). The total activation of the memory is the sum of the decaying traces (gray line).


```{r, fig.width=4, fig.height=2.5}
trace_odds <- function(t, te, d, ceil=5) {
  y <- (t - te) ^ -d
  y[t < te] <- 0
  y[y == Inf] <- ceil
  y
...
activation <- function(t, traces=c(0), sof=0.3, c=0.5) {
  y <- rep(0, length(t))
  d <- NULL
  for (te in traces) {
    d <- c * y[t==te] + sof
    y <- y + trace_odds(t, te, d)
  }
  log(y)
}
...
for (te in traces) {
  d <- bias * y[t==te] + sof
  odds <- trace_odds(t, te, d, ceil=3)
  partial <- tibble(Time=t, Odds = odds, SoF = rep(d, length(t)), Name = paste("Trace", counter))
  if (is.null(data)) {
    data <- partial
  } else {
    data <- rbind(data, partial)
  }
  counter <- counter + 1
  y <- y + odds
}

data$Odds[data$Odds == 0] <- NA
data$SoF <- round(data$SoF, 2)

memory_data <-tibble(Time=t, Activation = y, Name="Memory") 

point_data <- tibble(Time = traces,  Odds=3,
                     Name = paste("Trace ", 1:3),# ", ",  "d = ", unique(data$SoF), sep=""), 
                     SoF = unique(data$SoF))

p1 <- ggplot(data, aes(x=Time, y=Odds)) +
  scale_color_viridis(option="plasma", discrete=T, end=0.75) +
  geom_line(data=memory_data, 
            aes(x=Time, y=Activation, col=Name),
            col="black", size=1.5, alpha=0.25, show_guide=T) +
  geom_line(aes(group=SoF, color=as.factor(SoF)), show_guide=F) +
  geom_point(data=point_data, aes(x=Time, y=Odds, col=as.factor(SoF)), show_guide=F) +
  geom_label_repel(data=point_data, aes(x=Time, y = Odds, label=Name, col=as.factor(SoF)), show_guide=F) +
  ylab("Acxtivation") +
  xlab("Time (s)") +
#  scale_y_log10()
  annotate(geom="label", x=35, y=1, label="Memory", 
           col="grey", show_guide=F, label.size=1) +
  #labs(col="Trace Decay") +
  ggtitle("Memory and Component Traces") +
  theme_minimal() +
  theme(legend.position = "none",
        axis.text.y = element_blank())

p1 
ggsave("figures/model.png")
...
The activation of each trace decays as a power function of time, and the activation of $m$ is the log sum of the decaying activations of all its traces:

$A(m, t) = \log \sum_i (t - t_i)^{-d_i}$

where $t_i$ and $d_i$ are, respectively, the time of the creation of the i-the trace and its characteristic decay rate. A trace’s decay rate, in turn, is related to the m’s activation at the moment each trace is created, thus capturing the fact that traces with higher initial activation decay faster than traces with lower activation (Pavlik & Anderson, 2005). Specifically:

$d_i = e^{A(m, t_i)} + \phi$

with $A(m, t_i)$ being the activation of $m$ at the moment the $i$-th trace was encoded. Thus, if the times at which a memory m has been encoded, re-encoded, and retrieved are known and controlled for, its activation depends only on $\phi$, which is the only free parameter in the above equations and, by determining the decay of each memory, was taken to represent an individual-specific __speed of forgetting__.
...
braintype %<>% mutate(Type = "Storage") %>%
  mutate(Type = if_else(region %in% c("Control A",
                        ...
                        "Retrieval", 
                        Type)) %>%
  mutate(Type = if_else(region %in% c("Default A",
                                      "Default B",
                                      "Default C"),
                        "DMN", 
                        Type))
...
These are the different connectivity patterns predicted by the two theories.

```{r, fig.width=7, fig.height=1.8}
classified <- tibble(region = networks) %>%
  mutate(Type = "Storage") %>%
  mutate(Type = if_else(region %in% c("Control A",
                                      "Control B",
                                      "Limbic B",
...
ggplot(classified, aes(fill=Type)) +
  geom_brain(atlas = ggsegYeo2011::yeo17, 
             position = position_brain("horizontal"),
             color="white", size=0.2) +
  scale_fill_jama(na.value="grey") +
  labs(fill = "Type") +
  ggtitle("Large Scale Networks Associated with Memory Retrieval or Storage") +
  theme_void() +
  theme(legend.position = "bottom") +
  theme(legend.key.width = unit(1.5, "cm"))
```
...
ggsave("figures/associative-vs-control.png")
```
