---
title: "Predicting Python Programming Accuracy from Resting-State fMRI"
output:
  html_document:
    code_folding: hide
    theme: yeti
...
# Predicting Python Programming Accuracy from Resting-State Functional Connectivity

The goal of this project is to test whether individual differences in programmig accuracy can be decoded from resting state fMRI.
...
We will also need Python-compatible matrices for visualization. They can be created ex-novo by setting this parameter to T
...
Now, we need to remove some columns from the hyper-large _X_ matrix of regressors, and define proper groupings for Lasso.
...
# The real set of regressors:
R <- C[,as_vector(I)]
```

## Load the dependent variable $Y$

Now we need to load the dependent variable, which, in this case, is the programming accuracy. To do this, we need to merge two datasets, the list ofparticipants' meta-data in `participats.tsv` and the list of Python-related outcomes in `Python_Outcomes.csv`.

Note that we could not measure the programing accuracy $A_P$ for all participants, so we have to keep track of which rows in the $X$ regressor matrix we want to exclude from our Lasso analysis.

```{r}
dvs1<- read_tsv("../rsfmri/participants.tsv")
dvs2 <- read_csv("PythonOutcomes.csv") %>%
  filter(Subject != "NO RS") %>%
  filter(Subject != "sub-048")
dvs <- full_join(dvs1, dvs2)
keep <- !is.na(dvs$ProgrammingAccuracy)  & !dvs$Subject %in% NOFLY
Y <- dvs$ProgrammingAccuracy[keep]
X <- R[keep,]
```

The regressors $X$ is certainly multi-collinear. To gather a sense of how much it is so, we can plot the distribution of the correlations among regressors. They should be normally distributed around zero, and the smaller is the standard deviation, the better it is.
...
```{r}
dependent <- as_tibble(data.frame(ProgrammingAccuracy = Y))
#d3 <- pal_d3()
#kol = d3(7)

ggplot(dependent, aes(x=ProgrammingAccuracy, fill=cut(ProgrammingAccuracy, 10))) +
  geom_histogram(col="white", alpha=0.5, bins=30) +
  scale_fill_viridis(option = "plasma", discrete=T) +
  geom_vline(xintercept = mean(dependent$alpha)) +
  xlab(expression(paste("Programming Accuracy (", A[P], ")")))+
  ylab("Number of Participants") +
  ggtitle("Distribution of Programming Accuracy") +
  theme_pander() +
...
write_csv(connectome, file="chantel.csv")
save(fit, fit.cv, X, Y, order, I, G, file="chantel.RData")
...
```{r}
chisq.test(roi_stats$N*length(lROIs), p=roi_stats$N)
```
...
The difference in distribution is  
...
Here are the connections in the brain:

![Connectivity](figures/prog_acc_pretty.png)


And here is their relative network importance, visualized as a relative size:

![Importance](figures/prog_acc_importance_size.png)
...
ggplot(lconnections, aes(x = reorder(Connection, Beta), y = Beta)) +
  ...
  stat_summary(fun.data = "mean_cl_boot", 
               col="black", geom="errorbar", width=0.2) +
