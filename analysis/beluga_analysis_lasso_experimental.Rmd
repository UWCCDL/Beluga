...
ggplot(total_stats2, aes(x="", y=P, fill=connection)) +
  geom_bar(stat = "identity", col="grey", width=1) +
  facet_grid(~Source, labeller = label_both) +
  coord_polar("y", start=0) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 2L)) +
...
Then, we examine the VIF:

```{r}

dfX <- data.frame(cbind(Y, X[,betas != 0]))
names(dfX) <- c("Y", paste("X", 1:25, sep=""))
mod<-lm(Y~X1 + X2 +X3 + X4 +X5 +X6 + X7 + X8 +X9 + X10 + X11 + X12 +X13 + X14 +X15 +X16 + X17 + X18 +X19 + X20 + X21 + X22 +X23 + X24 +X25, data=dfX)
vif(mod)
