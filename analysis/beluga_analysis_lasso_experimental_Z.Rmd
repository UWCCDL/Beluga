...
```{r}
pow264 <- power2011
names(pow264)[1:4] <- c("ROI.Name",  "x.mni", "y.mni", "z.mni")
pow264$x.mni <- as.integer(pow264$x.mni)
pow264$y.mni <- as.integer(pow264$y.mni)
pow264$z.mni <- as.integer(pow264$z.mni)
```
...
```{r}
N <- nrow(pow264)
connMatrix <- matrix(rep(0, N**2), ncol=N) 

set_beta <- function(conn, beta) {
  temp <- str_split(conn, "-")
  i <- as.integer(temp[[1]][1])
  j <- as.integer(temp[[1]][2])
  connMatrix[i, j] <<- beta
  connMatrix[j, i] <<- beta
}

vset_beta <- Vectorize(set_beta)

connectome %>%
  mutate(Done <- vset_beta(connection, Beta))

#vset_beta(connectome$connection, connectome$Beta)
...
p2 <- brainconn(atlas=pow264, 
          #conmat=as.matrix(connMatrix !=0)+0,
          conmat=connMatrix,
          #node.size = 3, 
          view="bottom", 
          #node.color="network", 
...
ggsave(p, width=4, height=4, filename="p1.png")
ggsave(p2, width=6, height=4, filename="p2.png")
```
