...
p <- ggplot(wcomparison, aes(y=Predicted, x=Observed, 
                             col=(100*(Predicted-Observed)/Observed))) +
...
#  guides(col=guide_legend("Error")) +
  ...
  annotate("text", x=0.3, y=0.35,
           label=paste("r(33) =", 
                       floor(100*cor(Y, prediction))/100)) +
...
roi_stats <- power2011[lROIs,] %>%
  group_by(NetworkName, Color) %>%
  summarise(N=length(Color)/43) %>%
...
ggplot(total_stats, aes(x="", y=N, fill=NetworkName)) +
  ...
  geom_text_repel(aes(label=percent(N, .01)), col="white",
...
```{r}
chisq.test(roi_stats$N*43, p=noi_stats$N)
```
...
coi$connection <- vconnectivity(coi$network_names)

coi_stats <- coi %>% 
  group_by(connection) %>% 
  summarise(N=length(index), P=length(index)/9045)

connectome$connection <- vconnectivity(connectome$network_names)
connectome_stats <- connectome %>%
  group_by(connection) %>% 
  summarise(N=length(index), P=length(index)/29)
  
```
# Testing the validity of the Lasso model

To test the validity, we remove the 29 connections with significant beta values, and check whether the results are still significant. 
