...
With all the pieces in place, we can now do the Group Lasso:
...
plot(fit.cv)
plot(fit, group=T, col=inferno(length(unique(G$network))))

network_names_mapping <- order %>%
  group_by(network) %>%
  summarise(network=Mode(network), name=Mode(network_names))

uG <- unique(G$network)
Gnames <- sapply(uG, function(x)
{
  network_names_mapping$name[network_names_mapping$network == x]
}
)
legend(x="topright", legend=Gnames, col=inferno(length(Gnames)), lty=1, lwd=2)
```
